...
const invalidateCache = function (id = undefined) {
  return getCache().then((cache) => {
    return cache.del("categories").then(() => !id || cache.del(`cat___${id}`));
  });
};
...
exports.syncUser = function (user) {
  var groups = user.memberGroups.map((group) => {
    return group.cn;
  });
  ...
  var payload = new Buffer.from(querystring.stringify(params), "utf8").toString(
    "base64"
  );
  hmac.update(payload);
  var postParams = {
  ...
  return exports.post("admin/users/sync_sso", postParams).then(() => {
    return user;
  });
};
...
exports.getUser = function (uid, fetchEmail = false) {
  return exports
    ...
    .then((response) => {
      if (fetchEmail) {
        return exports
          .get("users/" + response.user.username + "/emails.json", {
            context:
              "admin/users/" + response.user.id + "/" + response.user.username,
          })
          .then((emailObject) => {
...
exports.deleteOrSuspendUser = function (uid) {
  return exports
    ...
    .then((discourseUser) => {
      if (discourseUser) {
        return exports
          .del("admin/users/" + discourseUser.id + ".json", {
            context:
              "/admin/users/" + discourseUser.id + "/" + discourseUser.username,
          })
          .then(() => {
            return { deleted: true };
          })
          .catch((error) =>
            exports
              .put("admin/users/" + discourseUser.id + "/suspend", {
                suspend_until: "3018-01-01",
                reason: "Gelöscht durch User Tool",
              })
              .then(() => {
    ...
    .catch((error) => {
      throw "Discourse user konnte nicht gelöscht werden: " + error;
    });
};
  ...
  if (!resolvedGroups.includes(group.dn)) {
    ...
    group.member.forEach((dn) => {
      if (ldaphelper.isGroup(dn)) {
        var subGroup = groups.find((group) => {
          return group.dn === dn;
        });
...
var getGroupId = function (name) {
  return exports.get("groups/" + name + ".json", {}).then(function (result) {
          ...
          if (group.cn && group.cn !== "admin") {
            return exports
              ...
              .then((oldMembers) => {
                ...
                return ldaphelper.dnToUid(newMembers).then((newMembers) => {
  ...
  return exports
    ...
    .then((response) => {
      if (!response.active || !response.success) {
        throw (
          "Benutzer*in erstellt, konnte aber nicht aktiviert werden: " +
          response.message
        );
      } else {
        return exports
          .put("u/" + username + ".json", { title: title })
          .then((response) => {
            return response.user;
          });
      }
...
exports.getCachedCategory = function (id) {
  return getCache()
    .then((cache) => cache.get(`cat___${id}`))
    .then((category) => {
      //console.log('cache hit', id, category !== undefined)
      return (
        category ||
        exports.get("c/" + id + "/show.json").then((response) => {
          return getCache()
            .then((cache) => cache.set(`cat___${id}`, response.category))
...
exports.getCategory = function (id) {
  return exports.getCachedCategory(id).then((cat) => {
    return exports.getCategories().then((categories) => {
...
exports.getCategories = function () {
  return getCache()
    .then((cache) => cache.get("categories"))
    .then((categories) => {
      return (
        categories ||
        exports
          ...
          .then((categoryTree) => {
            ...
            categoryTree
              .filter(
                (cat) =>
                  ...
                  !["team", "lounge", "feedback"].includes(cat.slug)
              )
            ...
            return getCache()
              .then((cache) => cache.set("categories", categoriesFlat))
              .then(() => categoriesFlat);
          })
      );
    });
...
exports.populateCategoryGroups = function (categories) {
  return Promise.each(categories, (category) => {
    return exports.getCachedCategory(category.id).then((category) => {
...
exports.createCategory = function (category) {
  return buildCategory(category)
    ...
    .then((catObject) =>
      invalidateCache().then(() => exports.post("categories", catObject))
    );
};

exports.updateCategory = function (category) {
  return buildCategory(category).then((catObject) =>
    invalidateCache(category.id).then(() =>
      exports.put("categories/" + category.id, catObject)
    )
  );
};

exports.deleteCategory = function (id) {
  return exports.del("categories/" + id, {}).then(() => invalidateCache(id));
};

exports.getGroupMembers = function (groupName) {
  return exports
    .get("groups/" + groupName + "/members.json?limit=1000")
    .then((result) => {
...
exports.addGroupMembers = function (groupId, newMembers) {
  return Promise.resolve().then(() => {
    if (newMembers.length > 0) {
      return exports.put("groups/" + groupId + "/members.json", {
        usernames: newMembers.join(","),
      });
    } else {
      return;
    }
...
exports.removeGroupMembers = function (groupId, members) {
  return Promise.resolve().then(() => {
    if (members.length > 0) {
      return exports.del("groups/" + groupId + "/members.json", {
        usernames: members.join(","),
      });
    } else {
      return;
    }
