...
exports.createAndSaveToken = function (
  currentUser,
  data,
  validPeriod = 7 * 24,
  type = undefined
  ...
  return readStore().then((store) => {
    ...
    return saveStore(store).then((store) => {
      return store[token];
    });
  });
};
...
exports.refreshToken = function (currentUser, token, validPeriod = 7 * 24) {
  return readStore().then((store) => {
    if (!store[token]) {
      ...
      store[token].expires.setHours(
        store[token].expires.getHours() + validPeriod
      );
      store[token].currentUser = {
        uid: currentUser.uid,
        dn: currentUser.dn,
...
exports.getTokenByData = function (value, path, type = undefined) {
  return readStore().then((store) => {
    var foundToken = Object.keys(store).find((token) => {
      return (
        (!type || store[token].type == type) &&
        store[token].data[path].toLowerCase() == value
      );
    });
    if (foundToken) {
...
exports.deleteToken = function (token) {
  return readStore().then((store) => {
    if (store[token]) {
      const data = { ...store[token].data };
      delete store[token];
      return saveStore(store).then(() => data);
    } else {
      throw "Token nicht gefunden";
    }
  });
