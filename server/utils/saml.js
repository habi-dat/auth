...
const buildLoginResponseTemplate = () => {
  var attributes = "";
  ["username", "uid", "place", "email", "title"].forEach((attribute) => {
...
const buildUser = (user) => {
  console.log(
    "SAML groups",
    user.memberGroups.map((group) => group.cn.replaceAll(",", "\\,")).join(",")
  );
  return {
    uid: user.uid,
    name: user.cn,
...
const createTemplateCallback = (_idp, _sp, user, requestId) => (template) => {
  ...
  const tvalue = {
    ...
    attr_groups: user.memberGroups
      .map(
        (group) =>
          `<saml:AttributeValue xsi:type="xs:string">${group.cn}</saml:AttributeValue>`
      )
      .join(""),
  };
  return {
...
const sessionRemoveLoggedInApp = (req, appId) => {
  if (
    req.session.samlLoggedInApps &&
    req.session.samlLoggedInApps.includes(appId)
  ) {
    req.session.samlLoggedInApps = req.session.samlLoggedInApps.splice(
      req.session.samlLoggedInApps.indexOf(appId),
      1
    );
  }
};
...
exports.createLoginResponse = (req, app, request, type, user) => {
  ...
  return idp
    .createLoginResponse(
      ...
      type,
      buildUser(user),
      createTemplateCallback(idp, sp, user, request.extract.request.id)
    )
    .then((response) => {
...
exports.loginRedirect = (req, app, request) => {
  return Promise.resolve().then(() => {
    return (
      "/#/login?requestId=" + request.extract.request.id + "&appId=" + app.id
    );
  });
};
...
exports.initiateLogoutFlow = (
  ...
  return Promise.resolve().then(() => {
    req.session.samlLogoutFlow = {
      ...
      queue: sessionGetLoggedInApps(req).filter(
        (appId) => appId !== initiatorId
      ),
    };
...
exports.createLogoutRequest = (req, app, type) => {
  return Promise.resolve().then(() => {
    const sp = createSp(app);
    const request = idp.createLogoutRequest(sp, "redirect", {
      logoutNameID: req.user.uid,
    });
...
exports.parseLogoutRequest = (req, app, type) => {
  return idp.parseLogoutRequest(createSp(app), type, req).then((request) => {
    return exports.initiateLogoutFlow(req, app.id, request).then((sloFlow) => {
      if (sloFlow.queue.length === 0) {
        return exports.createLogoutResponse(req, app, request, "redirect");
      } else {
        return apps.getApp(sloFlow.queue[0]).then((nextApp) => {
...
exports.parseLogoutResponse = (req, app, type) => {
  return idp.parseLogoutResponse(createSp(app), type, req).then((response) => {
    ...
    if (sloFlow.queue.length === 0) {
      if (sloFlow.initiatorId) {
        return apps
          .getApp(sloFlow.initiatorId)
          .then((initiatorApp) =>
            exports.createLogoutResponse(
              req,
              initiatorApp,
              sloFlow.initiatorRequest,
              "redirect"
            )
          );
      } else {
