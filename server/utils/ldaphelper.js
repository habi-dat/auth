...
const userAttributes = ['uid', 'dn', 'cn', 'ou', 'l', 'mail', 'preferredLanguage', 'description', 'title']
...
exports.fetchUser = function (dn, parentGroups = true, fetchGroupDetails = false) {
  return exports.fetchObject(dn, userAttributes)
    .then((user) => {
...
exports.populateUserGroups = function (user, parentGroups = true, fetchGroupDetails = false) {
...
exports.populateUserTitle = function (user) {
  return Promise.resolve()
    .then(() => {
      if (!!user.ou) {
        return exports.fetchObject(user.ou, ['o'])
          .then(group => {
            user.title = group.o;
            return user;
          })
...
exports.fetchUsers = function (userDns = undefined) {
  return new Promise((resolve, reject) => {
    ...
    client.search('ou=users,' + config.server.base, opts, function (err, res) {
      res.on('searchEntry', function (entry) {
        if (entry.object.cn && (!userDns || userDns.includes(entry.object.dn))) {
          entries.push(entry.object);
        }
      });
      res.on('error', function (err) {
...
exports.fetchParentGroups = function (groupDn) {
  return exports.fetchObjectsByFilter('(member=' + groupDn + ')', 'ou=groups,' + config.server.base, groupAttributes);
}

exports.fetchGroup = function (dn) {
  return exports.fetchObject(dn, groupAttributes)
    .then(group => {
      group.member = exports.ldapAttributeToArray(group.member);
      group.owner = exports.ldapAttributeToArray(group.owner);
      ...
      return Promise.join(exports.fetchGroups(subGroups),
...
exports.createUser = function (user) {
  return new Promise((resolve, reject) => {
    ...
    user.objectClass = ['inetOrgPerson', 'posixAccount', 'top', 'organizationalPerson'];
    user.gidNumber = 500;
...
exports.updateUserOu = function (oldDN, newDN) {
  return exports.fetchObjectsByFilter('(ou=' + oldDN + ')', 'ou=users,' + config.server.base)
    .then(users => {
...
exports.updateUserTitle = function (grouDn, o) {
  return exports.fetchObjectsByFilter('(ou=' + grouDn + ')', 'ou=users,' + config.server.base)
    .then(users => {
      return Promise.map(users, user => {
  ...
  return exports.fetchUser(dn)
    ...
    .then(oldUser => {
      var updateKeys = Object.keys(user).filter(key => {
        return (['cn', 'ou', 'title', 'l', 'preferredLanguage', 'description', 'mail'].includes(key) && user[key] !== undefined);
      })
...
exports.removeUserFromGroups = function (userDn, attribute, groupDns) {
  return Promise.map(groupDns, groupDn => {
    return exports.fetchObject(groupDn)
      .then(group => {
        var parameters = {};
        var newUsers = exports.ldapAttributeToArray(group[attribute]).filter(element => {
...
exports.syncUserGroups = function (userDn, attribute, oldGroupDns, newGroupDns, currentUser) {
  return Promise.resolve()
    .then(() => {
      ...
      var removeGroupDns = oldGroupDns.filter(oldGroupDn => {
        return !newGroupDns.includes(oldGroupDn) && (currentUser.isAdmin || currentUser.owner.includes(oldGroupDn));
      })
      ...
      var addGroupDns = newGroupDns.filter(newGroupDn => {
        return !oldGroupDns.includes(newGroupDn) && (currentUser.isAdmin || currentUser.owner.includes(newGroupDn));
      })
      return exports.addUserToGroups(userDn, attribute, addGroupDns)
        .then(() => exports.removeUserFromGroups(userDn, attribute, removeGroupDns))
    })
}
...
exports.createGroup = function (group) {
  return Promise.resolve()
    .then(() => {
      ...
      return exports.add(exports.groupCnToDn(group.cn), entry)
        .then(() => exports.syncParentGroups(exports.groupCnToDn(group.cn), group.parentGroups))
    })
}
  ...
  return Promise.resolve()
    .then(() => {
      if (!parentGroups) {
        ...
        return exports.fetchParentGroups(dn)
          .then(oldParentGroups => {
            oldParentGroups = oldParentGroups.map(g => g.dn);
            var addParents = parentGroups.filter(dn => !oldParentGroups.includes(dn));
            var delParents = oldParentGroups.filter(dn => !parentGroups.includes(dn));
            return exports.addUserToGroups(dn, 'member', addParents)
